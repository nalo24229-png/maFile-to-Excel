...
DATA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "Input Data Here"))
...
OUTPUT_ROOT = os.path.abspath(os.path.join(BASE_DIR, "..", "Your Ready Accounts"))
DIR_TOGETHER = os.path.join(OUTPUT_ROOT, "All accounts together")
DIR_SEPARATE = os.path.join(OUTPUT_ROOT, "Each account separately")
...
def parse_proxies(file_path):
    if not os.path.exists(file_path):
        print(
            f"[-] WARNING: Proxy file '{file_path}' not found. Building without proxies."
        )
        return []
    res = []
    with open(file_path, "r", encoding="utf-8") as f:
...
def pack_xml_to_xlsx(xml_content, target_xlsx_path):
    ...
    with open(
        os.path.join(temp_worksheets, "sheet1.xml"), "w", encoding="utf-8"
    ) as f:
        f.write(xml_content)
    ...
    subprocess.run(
        winrar_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
...
def main():
    if not os.path.exists(WINRAR_PATH) or not os.path.exists(TEMPLATE_FILE):
        print("[-] ERROR: System configuration files or WinRAR not found!")
        return
    ...
    if not os.path.exists(MAFILES_DIR):
        print("[-] ERROR: Mafiles directory is missing!")
        return

    files = [f for f in os.listdir(MAFILES_DIR) if f.endswith(".maFile")]
    files.sort()
    if not files:
        print("[-] ERROR: No .maFile documents found to process!")
        return
    ...
    for file_name in files:
        try:
            ...
            steam_id = session.get("SteamID", "") or mafile_json.get(
                "steamid", ""
            )
            ...
            password = txt_passwords.get(login, "") or mafile_json.get(
                "password", ""
            )
            safe_content = (
                content.replace("&", "&amp;")
            ...
            with open(info_txt_path, "w", encoding="utf-8") as info_f:
                info_f.write(f"Account Login: {login}\n")
                info_f.write(f"Account Password: {password}\n")
                info_f.write(f"Auto Trade URL: {calculated_trade_url}\n")
                info_f.write(
                    f"Assigned Proxy: {proxy if proxies_list else 'None'}\n"
                )
            ...
            single_xlsx_path = os.path.join(
                bot_folder, f"market_bot_{login}.xlsx"
            )
    ...
    print(f"[ info ] Total bots packed into tables: {valid_idx}")
